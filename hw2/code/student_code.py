...
class CustomConv2DFunction(Function):
    ...
    def forward(ctx, input_feats, weight, bias, stride=1, padding=0):
        ...
        # 2 img
        # 7x7 K, 3 C, each patch 147;
        # 36 block, 7x7 on 12x12
        # print(unfold_input.size())

        unfold_weight = weight.view(weight.size(0), -1)
        # weight.size(0) = C_o, then reshape the weight
        unfold_output = unfold_weight.matmul(unfold_input)

        temp = unfold_output.transpose(0, 1)
        # add bias; bias size of C_o, transpose
    ...
    def backward(ctx, grad_output):
        ...
        # unfold weight; weight.size(0) = C_o, then reshape the weight
        ...
        # calculate fold grad input based on the unfold_grad
        # input grad
...
class PGDAttack(object):
    ...
    def perturb(self, model, input):
        ...
        #################################################################################
        # Fill in the code here
            # Find the least confident label
            pred = model(output)
            least, _ = torch.min(pred, 1)
            
            # Backward and get sign of gradient
            least.backward(torch.ones(least.shape, device=torch.device('cuda')))
            sign = torch.sign(output.grad.data)
            ...
            # Empty grad of output before next iteration
            output.grad.zero_()
        #################################################################################
...
class GradAttention(object):
    ...
    def explain(self, model, input):
        ...
        pred = model(input)
        maximum, _ = torch.max(pred, 0)
        # pred = pred.gather(1, maxima.view(-1, 1)).squeeze()
        pred = maximum.squeeze()
        # Backward and get gradient
        pred.backward(torch.ones(pred.shape, device=torch.device('cuda')))
        abs_grad = input.grad.abs()

        # Pick the maximum values across three color channels
        output, _ = torch.max(abs_grad, dim=1)
        output = output.view(output.shape[0], 1, output.shape[1], output.shape[2])
        #################################################################################
